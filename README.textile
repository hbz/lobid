...
This software implements "http://lobid.org":http://lobid.org, the LOD service offered by "hbz":http://www.hbz-nrw.de.
...
The @lobid/lodmill@ repo contains code for working with raw data and its transformation to LOD, based on the Culturegraph Metafacture toolkit (@lodmill-rd/@). It also contains code for processing RDF with Hadoop and for indexing it in Elasticsearch (@lodmill-ld/@). This repo contains a web app based on the Play framework that interacts with the resulting Elasticsearch index. It provides an HTTP API to the index and a UI for documentation and basic sample usage. See below for details on the data workflow.
...
The instructions above will set up a local API and frontend instance. For information on how we have set up our Hadoop and Elasticsearch backend clusters see @README.textile@ in the @lobid/lodmill@ repo, folder @lodmill-ld@.
